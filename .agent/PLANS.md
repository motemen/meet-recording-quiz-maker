...
- Env naming: `GOOGLE_DRIVE_FOLDER_ID`, `GEMINI_MODEL`, `GOOGLE_GENERATIVE_AI_API_KEY`, `QUIZ_ADDITIONAL_PROMPT`, `FIRESTORE_COLLECTION`, `GOOGLE_ALLOWED_DOMAIN`, `GCLOUD_PROJECT`, `PORT`.

## Milestones
1) **Scaffold (done)**: Express app with routes `/tasks/scan`, `/tasks/process`, `/manual`, `/files/:fileId`, minimal UI; clients for Drive/Forms/Gemini; Firestore repo; config validation.
...
## Open Items
- Decide auth mechanism for UI (`/manual`): basic auth vs IAP.
